...
**
** Upper
** This class contains the upper level functions and methods of MyDbConns.
*/
...
// It requires the connections password and its decrypted content.
    ...
    {
      if ( d != null && c != null && q != null && us != null )
      {
...
// Calculate the elapsed time when it is finished.
                        elapsedFormatted = us . calculateElapsed ( new Date ( ) . getTime ( ) - startDate . getTime ( ) ) ;
...
// We should release these too.
      ...
      {
        systemexit ( "Error - one of these is null: d|c|q|us, factoryQuery" ) ;
      }
    }
  }
/*
...
// If a connection is currently in use then it is unable to delete it, message to the user.
                      outprintln ( fold + connection . getKey ( ) + spaceChar + messageThisConnectionIsInUse + us . joinArrayListInteger ( connection . getValue ( ) , sep1 ) ) ;
                    }
                    else
                    {
...
// These are releasable too.
      ...
      {
        systemexit ( "Error - us is null, deleteConnectionsByDbType" ) ;
      }
    }
  }
/*
