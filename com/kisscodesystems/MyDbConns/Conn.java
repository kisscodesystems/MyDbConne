...
*/
  protected final void clearCharArrays ( )
  {
    if ( us != null )
    {
      us . clearCharArray ( passwordFromInputOriginal , zeroChar ) ;
      us . clearCharArray ( passwordFromInputVerified , zeroChar ) ;
      us . clearCharArray ( passwordForConnections , zeroChar ) ;
      us . clearCharArray ( fileContentConnectionsOrig , zeroChar ) ;
      us . clearCharArray ( fileContentConnectionsTrim , zeroChar ) ;
    }
    else
    {
      systemexit ( "Error - us is null, clearCharArrays" ) ;
    }
  }
/*
...
*/
  protected final void clearByteArrays ( )
  {
    if ( us != null )
    {
      us . clearByteArray ( slConnections , nullByte ) ;
      us . clearByteArray ( ivConnections , nullByte ) ;
    }
    else
    {
      systemexit ( "Error - us is null, clearByteArrays" ) ;
    }
  }
/*
...
// These are not in use.
    ...
    {
      systemexit ( "Error - one of these is null: us|fileName, readPassword" ) ;
    }
// Not usable.
    fileName = null ;
  }
...
// Converting to byte array and place this into the bytes array
        bytes = us . toBytesASCII ( fileContentConnectionsTrim ) ;
...
// These byte arrays have to be cleared!
        us . clearByteArray ( encryptedBytes , nullByte ) ;
        encryptedBytes = null ;
        us . clearByteArray ( bytes , nullByte ) ;
        bytes = null ;
      }
      else
      {
...
// These are releasable references.
    ...
    {
      systemexit ( "Error - one of these is null: us|fileName, saveFile" ) ;
    }
...
// Do the conversion.
            char [ ] tempChar = us . toCharsASCII ( decryptedBytes ) ;
...
// This temporary character array will be not used any more, can be cleared.
            us . clearCharArray ( tempChar , zeroChar ) ;
            tempChar = null ;
            us . clearByteArray ( decryptedBytes , nullByte ) ;
            decryptedBytes = null ;
          }
          else
          {
...
// We are not started yet because some file is missing.
    ...
    {
      systemexit ( "Error - one of these is null: us|fileName, getFileContent" ) ;
    }
...
// And has to be cleared by the space filling character.
          us . clearCharArray ( movedPart , zeroChar ) ;
...
// Not in use any more.
      ...
      {
        systemexit ( "Error - us is null, shiftFileContent" ) ;
      }
    }
  }
/*
