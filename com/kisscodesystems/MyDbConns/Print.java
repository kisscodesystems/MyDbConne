...
*/
  ...
  {
    if ( typeToList != null && dbtype != null && us != null )
    {
...
// Printing out the header. (dbtype + type of connections being listed)
              outprint ( newLineString + fold + us . pad ( dbtype + stringToDisplay , appMaxQueryIdTitleConnnaWidth + ( fold2 . length ( ) - fold . length ( ) ) , spaceChar ) ) ;
...
// Let's create the element to be listed later.
                        listElementToSortedList = newLineString + fold2 + us . pad ( connection . getKey ( ) , appMaxQueryIdTitleConnnaWidth , spaceChar ) ;
                        if ( ! typeToList . equals ( typeToListInactive ) )
                        {
// Let's append the queries belongs to this connection if the active or all connections are expected.
                          listElementToSortedList = listElementToSortedList + sep9 + us . joinArrayListInteger ( connection . getValue ( ) , sep1 ) ;
                        }
...
// These are releasable now.
    ...
    {
      systemexit ( "Error - one of these is null: dbtype|us|typeToList, listConnectionsByDbtype" ) ;
    }
  }
/*
...
// Adding the current to the list.
                        sortedList . add ( newLineString + fold2 + us . pad ( query . getKey ( ) + " (" + getQueryTitle ( Integer . parseInt ( query . getKey ( ) ) ) + ")" , appMaxQueryIdTitleConnnaWidth , spaceChar ) + sep9 + query . getValue ( ) ) ;
                      }
                    }
                    else
                    {
...
// Not used references.
    ...
    {
      systemexit ( "Error - one of these is null: dbtype|us|typeToList, listQueriesByDbtype" ) ;
    }
  }
/*
...
// Go thru on the cols array: finalizing the maximum length of the columns.
                    ...
                    {
                      val = us . getVal ( rs , colname ) ;
                      valStr = us . getValStr ( val , nullStr ) ;
...
// The next lower case colname will be appended into the end of the current line.
                  line = line + ( ( resultFormatValue . equals ( resultFormatTxtValue ) && scrollable ) ? us . pad ( rsmd . getColumnName ( i ) . toLowerCase ( ) , colw [ i - 1 ] , spaceChar ) : rsmd . getColumnName ( i ) . toLowerCase ( ) ) ;
...
// Processing the line.
                      line = line + ( ( resultFormatValue . equals ( resultFormatTxtValue ) && scrollable ) ? us . pad ( valStr . replaceAll ( "\n" , "\\n" ) , colw [ j - 1 ] , spaceChar ) : valStr ) + fieldsep ;
                      j ++ ;
                    }
// csv or htm: the final separator has to be removed from the end of the line.
                    if ( resultFormatValue . equals ( resultFormatCsvValue ) || resultFormatValue . equals ( resultFormatHtmValue ) )
...
// Not in use.
      ...
      {
        systemexit ( "Error - us is null, constructResult" ) ;
      }
    }
    else
    {
  ...
  protected final boolean valToTheDiskSeparately ( ResultSet rs , String destfile , String colname , String coltype , String dbtype )
  {
    ...
    if ( coltype != null && dbtype != null )
    {
      if ( dbtype . equals ( dbTypeMysql ) )
      {
        ...
        {
          separately = us . getBlob ( rs , destfile , colname ) ;
        }
      }
      else if ( dbtype . equals ( dbTypeOracle ) )
      {
        ...
        {
          separately = us . getClob ( rs , destfile , colname , utf8 ) ;
        }
        else if ( coltype . equals ( raw ) || coltype . equals ( longraw ) )
        {
          separately = us . getRaw ( rs , destfile , colname , bufflength ) ;
        }
        else if ( coltype . equals ( bfile ) )
        {
          separately = us . getBfile ( rs , destfile , colname , bufflength ) ;
        }
      }
      ...
      else if ( dbtype . equals ( dbTypeDb2 ) )
      {
        ...
        {
          separately = us . getXml ( rs , destfile , colname , utf8 ) ;
        }
      }
...
// In use by default.
    boolean inUse = true ;
    if ( dbtype != null && connna != null && us != null )
    {
...
// Messages will go to the user if it has been requested.
        ...
        {
          inUse = true ;
          outprintln ( "" + ( messageIfNot ? "" : newLineChar ) + fold + messageThisConnectionIsInUse + us . joinArrayListInteger ( queryIds , sep1 ) ) ;
        }
      }
      else
      {
...
// Not in use any more.
    ...
    {
      systemexit ( "Error - one of these is null: dbtype|connna|us, isConnectionInUse" ) ;
    }
// Returning the usage.
    return inUse ;
  }
